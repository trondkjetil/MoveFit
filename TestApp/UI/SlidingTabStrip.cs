...
/// <summary>
/// Sliding tab strip.
	/// This class has control for the tab menu (located on the top) which is like a strip
	/// Linear layout that holds all the tabs. E.g Messages, Share are different tabs, on the strip
/// </summary>
	public class SlidingTabStrip : LinearLayout
	{
		...
		//The color on the tab green and blue
		...
		public void OnViewPagerPageChanged(int position, float positionOffset)
		{
			...
			//Where you are, and where you are going (tab)
			mSelectionOffset = positionOffset;
			// Redraws, calls onDraw
			this.Invalidate();
		}
		...
		protected override void OnDraw(Canvas canvas)
		{
			...
			if (tabCount > 0)
			{
				...
				if (mSelectionOffset > 0f && mSelectedPosition < (tabCount - 1))
				{
					...
					//Moving of tab. Offset is the distance. We check if we are at the end / next
				...
				//This draws to the canvas / view.Draws a rectangle
				...
				// Draws the border for the tab strip
				canvas.DrawRect(0, height - mBottomBorderThickness, Width, height, mBottomBorderPaint);
			}
		}


		//Blending the colors for the tabstrip transition. Increases the color on movement
